...
public class DtoSerializationTest extends MongoTestBase {
    ...
    // --- VulnerabilityStatsDTO Tests ---

    @Test
    public void testVulnerabilityStatsDTOExcludesLargeBlobs() {
        // The stats DTO constructor intentionally doesn't set description, recommendation, or details
        VulnerabilityStatsDTO dto = VulnerabilityStatsDTO.fromEntity(testVuln);

        // These fields don't exist on VulnerabilityStatsDTO by design
        // Just verify the DTO is created successfully without them
        assertNotNull("DTO should not be null", dto);
        assertEquals("ID should match", Long.valueOf(1L), dto.getId());
    }

    @Test
    public void testVulnerabilityStatsDTOIncludesLightweightFields() {
        VulnerabilityStatsDTO dto = VulnerabilityStatsDTO.fromEntity(testVuln);
    ...
    @Test
    public void testVulnerabilityStatsDTOWithRiskLevelLabels() {
        VulnerabilityStatsDTO dto = VulnerabilityStatsDTO.fromEntity(testVuln);

        // riskId 5 = Critical, 4 = High, 3 = Medium
        assertEquals("OverallStr should be Critical", "Critical", dto.getOverallStr());
        assertEquals("ImpactStr should be High", "High", dto.getImpactStr());
        assertEquals("LikelyhoodStr should be Medium", "Medium", dto.getLikelyhoodStr());
    }

    @Test
    public void testVulnerabilityStatsDTOWithNullOverall() {
        Vulnerability v = new Vulnerability();
        v.setName("No Overall");
        v.setAssessmentId(100L);
        ...
        VulnerabilityStatsDTO dto = VulnerabilityStatsDTO.fromEntity(v);
        assertNull("Overall should be null", dto.getOverall());
        // vulnStr(null) returns "Unassigned", not "Info"
        assertEquals("OverallStr should be Unassigned", "Unassigned", dto.getOverallStr());
    }

    @Test
    public void testVulnerabilityStatsDTOWithNullTracking() {
        Vulnerability v = new Vulnerability();
        v.setName("No Tracking");
        v.setAssessmentId(100L);
        v.setLevels(riskLevels);
    ...
    public void testVulnerabilityStatsDTOSerialization() throws Exception {
        ...
        // Verify large blobs are NOT in JSON
    ...
    // --- AssessmentStatsDTO Tests ---

    @Test
    public void testAssessmentStatsDTOWithVulnerabilities() {
        testAssessment.setVulns(Arrays.asList(testVuln));

        VulnerabilityStatsDTO vDto = VulnerabilityStatsDTO.fromEntity(testVuln);
        List<VulnerabilityStatsDTO> vulns = Arrays.asList(vDto);
        ...
        assertEquals("ID should match", Long.valueOf(100L), dto.getId());
        assertEquals("Name should match", "Test Assessment", dto.getName());
        assertNotNull("Vulnerabilities should not be null", dto.getVulnerabilities());
        assertEquals("Should have 1 vulnerability", 1, dto.getVulnerabilities().size());
    }

    @Test
    public void testAssessmentStatsDTOWithNullVulnerabilities() {
        AssessmentStatsDTO dto = AssessmentStatsDTO.fromEntity(testAssessment, null);

        assertNotNull("DTO should not be null", dto);
        assertNull("Vulnerabilities should be null", dto.getVulnerabilities());
        assertNull("Risk summary should be null", dto.getRiskSummary());
    }

    @Test
    public void testAssessmentStatsDTOWithEmptyVulnerabilities() {
        AssessmentStatsDTO dto = AssessmentStatsDTO.fromEntity(testAssessment, new ArrayList<>());

        // fromEntity only sets vulnerabilities when vulns is non-empty
        // Empty list results in null vulnerabilities
        assertNotNull("DTO should not be null", dto);
        assertNull("Vulnerabilities should be null when empty list passed", dto.getVulnerabilities());
        assertNull("Risk summary should be null when empty list passed", dto.getRiskSummary());
    }

    @Test
    public void testAssessmentStatsRiskSummary() throws Exception {
        List<VulnerabilityStatsDTO> vulns = new ArrayList<>();

        // Create vulns with different severity levels
        // riskId 5 = Critical, 4 = High, 3 = Medium
        Vulnerability v1 = new Vulnerability();
        v1.setId(1L);
        v1.setName("Vuln 1");
        v1.setOverall(5L);
        v1.setAssessmentId(100L);
        ...
        v3.setName("Vuln 3");
        v3.setOverall(4L);
        v3.setAssessmentId(100L);
        v3.setLevels(riskLevels);
        vulns.add(VulnerabilityStatsDTO.fromEntity(v3));

        AssessmentStatsDTO dto = AssessmentStatsDTO.fromEntity(testAssessment, vulns);

        Map<String, Integer> summary = dto.getRiskSummary();
        assertNotNull("Risk summary should not be null", summary);
    ...
    public void testAssessmentStatsDTOSerialization() throws Exception {
        ...
        assertTrue("JSON should contain Name", json.contains("\"Name\":\"Test Assessment\""));
        assertTrue("JSON should contain Type", json.contains("\"Type\":\"Web Application\""));
        assertTrue("JSON should contain Vulnerabilities", json.contains("\"Vulnerabilities\""));
    ...
    @Test
    public void testFromEntityWithNullVulnerabilityStats() {
        VulnerabilityStatsDTO dto = VulnerabilityStatsDTO.fromEntity(null);
        assertNull("Stats DTO should be null for null vulnerability", dto);
    }

    @Test
    public void testAssessmentStatsFromEntityWithNullAssessment() {
        AssessmentStatsDTO dto = AssessmentStatsDTO.fromEntity(null, new ArrayList<>());
        assertNull("Stats DTO should be null for null assessment", dto);
    }
