...
class AssessmentStats {
	...
	createCharts() {
		$.get(`GetStats?id=${id}`).done((resp) => {
			...
			this.severityNames.forEach( (_label, index) =>  {
				this.bgColors.push(resp.data.colors[index +l -1]+"22");
				this.borderColors.push(resp.data.colors[index +l -1]);
				}
			);
	...
	addVulnCount(severity){
		let indexToUpdate = this.severityNames.indexOf(severity);
		let count = this.vulnBarChart.data.datasets[0].data[indexToUpdate]
		this.vulnBarChart.data.datasets[0].data[indexToUpdate]= count +1;
		this.vulnBarChart.update();
	}
	removeVulnCount(severity){
		let indexToUpdate = this.severityNames.indexOf(severity);
		let count = this.vulnBarChart.data.datasets[0].data[indexToUpdate]
		this.vulnBarChart.data.datasets[0].data[indexToUpdate]= count -1;
		this.vulnBarChart.update();
