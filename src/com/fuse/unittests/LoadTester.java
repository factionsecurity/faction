...
public class LoadTester extends StrutsTestCase {
	...
	@Test
	public void t1createAssessment() throws Exception {

		/*
		 * ActionProxy proxy= getActionProxy("/Login.action"); Login login = (Login)
		 * proxy.getAction(); login.setUsername("admin");
		 * login.setPassword("password123"); String result = proxy.execute();
		 * assertEquals("Did not Redirect to assesment Queue" , "assessorQueue",
		 * result);
		 * 
		 * for(int j=0; j<1000; j++){ Long holdit; String uuid =
		 * UUID.randomUUID().toString(); Date now = new Date();
		 * System.out.println("================Assessment " + j);
		 * 
		 * 
		 * 
		 * 
		 ...
		 * 
		 * 
		 * proxy = getActionProxy("/portal/Engagement"); Engagement eng = (Engagement)
		 * proxy.getAction(); assertNotNull("Engagment Action is null", eng);
		 * eng.setAction("createAssessment"); eng.setAppid(uuid); eng.setAppName(uuid);
		 * eng.setSdate(now); eng.setEdate(now); eng.setAssessorId(new
		 * ArrayList<Integer>()); eng.getAssessorId().add(
		 * ((Long)u.getId()).intValue()); eng.setRemId(((Long)u.getId()).intValue());
		 * eng.setEngId(((Long)u.getId()).intValue()); eng.setCampId(camp1); //2017
		 * Assessments eng.setType(type1); //Manual Ethical hacking eng.setNotes(uuid);
		 * eng.setDistro("josh.summitt@fusesoft.co"); result = proxy.execute();
		 * assertEquals("Did not Redirect to assesment Queue" , eng.SUCCESS, result);
		 * holdit = eng.getUnitTestId();
		 * 
		 * 
		 * proxy = getActionProxy("/portal/DefaultVulns"); DefaultVulns dvulns =
		 * (DefaultVulns) proxy.getAction();
		 * assertNotNull("AddVulnerability Action is null", dvulns);
		 * dvulns.setTerms("xss"); dvulns.setAction("json");
		 * 
		 * result = proxy.execute(); assertEquals("Search Did not return results" ,
		 * "vulnsearch", result); List<DefaultVulnerability>searchedVulns =
		 * dvulns.getVulnerabilities(); assertNotNull("Null Vuln list", searchedVulns);
		 ...
		 * 
		 * proxy = getActionProxy("/portal/AddVulnerability"); AddVulnerability addVuln
		 * = (AddVulnerability) proxy.getAction(); addVuln.setAction("add");
		 * addVuln.setId(holdit.intValue()); addVuln.setDefaultTitle(
		 * ((Long)vuln2add.getId()).intValue()); addVuln.setId(holdit.intValue());
		 * addVuln.setImpact(3l); addVuln.setLikelyhood(3l); addVuln.setOverall(3l);
		 * addVuln.setTitle(uuid); result = proxy.execute();
		 * assertEquals("Vuln was not added Successfully" , "successJson", result);
		 * 
		 * //add a custom vuln proxy = getActionProxy("/portal/AddVulnerability");
		 * AddVulnerability custVuln = (AddVulnerability) proxy.getAction();
		 * custVuln.setAction("add"); custVuln.setId(holdit.intValue());
		 * custVuln.setDescription(uuid); custVuln.setRecommendation(uuid);
		 * custVuln.setId(holdit.intValue()); custVuln.setImpact(3l);
		 * custVuln.setLikelyhood(3l); custVuln.setOverall(3l);
		 * custVuln.setTitle("Custom-"+uuid); result = proxy.execute();
		 * assertEquals("Custom Vuln was not added Successfully" , "successJson",
		 * result);
		 * 
		 * 
		 * 
		 * //update assessment summaries and notes proxy =
		 * getActionProxy("/portal/Assessment"); AssessmentView av = (AssessmentView)
		 * proxy.getAction(); av.setId(""+holdit); av.setUpdate("true");
		 * av.setSummary(uuid); av.setRiskAnalysis(uuid); av.setNotes(uuid); result =
		 * proxy.execute(); assertEquals("Assessment Info was not added Successfully" ,
		 * "successJson", result);
		 * 
		 * 
		 * //add 10 exploit steps for(int i=0;i<10;i++){ proxy =
		 * getActionProxy("/portal/AddStep"); AddStep as = (AddStep) proxy.getAction();
		 * as.setAction("add"); as.setVulnid(custVuln.getUnittestVulnid());
		 * as.setEditor6(editorData); as.setAppId("app"+holdit); result =
		 * proxy.execute(); //assertEquals("Exploit Step not added" , "stepRedirect",
		 * result); }
		 * 
		 * /* //generate the report proxy = getActionProxy("/portal/Assessment");
		 * AssessmentView av2 = (AssessmentView) proxy.getAction();
		 * av2.setAction("genreport"); av2.setId(""+holdit); result = proxy.execute();
		 * assertEquals("Assessment Info was not added Successfully" , "successJson",
		 * result);
		 */
		/*
		 * File file = new File("C:/tmp/data.out"); FileInputStream fis = new
		 * FileInputStream(file); byte[] data = new byte[(int) file.length()];
		 * fis.read(data); fis.close();
		 * 
		 * EntityManager em = HibHelper.getInstance().getEM(); String str = new
		 * String(data); Assessment a = em.find(Assessment.class, holdit); FinalReport
		 * fr = new FinalReport(); fr.setBase64EncodedPdf(str); fr.setGentime(new
		 * Date()); fr.setFilename(uuid); a.setFinalReport(fr);
		 * 
		 * HibHelper.getInstance().preJoin(); em.joinTransaction(); em.persist(a);
		 * HibHelper.getInstance().commit(); em.close();
